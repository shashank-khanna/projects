...
class MovingAverageCrossOver(BackTestingBase):
    ...
    def get_signals(self):
        self.signals = pd.DataFrame()  # self.stock_data['Close']
        # self.signals['Close'] = self.stock_data['Close']
        self.signals['signal'] = 0.0
        self.signals['short_mavg'] = self.stock_data['Close'].rolling(window=self.SHORT_WINDOW, min_periods=1,
                                                                      center=False).mean()
        self.signals['long_mavg'] = self.stock_data['Close'].rolling(window=self.LONG_WINDOW, min_periods=1,
                                                                     center=False).mean()
        ...
        self.signals.set_index(self.stock_data.index, inplace=True)
    ...
    def plot_signals(self):
        ...
        ax1.plot(self.signals.index.map(mdates.date2num), self.stock_data['Close'])
...
if __name__ == '__main__':
    strategy = MovingAverageCrossOver('WMT', 2000)
